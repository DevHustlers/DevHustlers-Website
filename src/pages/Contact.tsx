        ...
        <>
          <section>
            <div className="max-w-5xl mx-auto px-3 sm:px-6 lg:px-0">
              <div className="grid grid-cols-1 md:grid-cols-2">
                {/* Message + Send + Follow us */}
                <div className="p-6 sm:p-8 border-b md:border-b-0 md:border-r border-border">
                  <label className="block text-[12px] font-medium text-muted-foreground mb-2 uppercase tracking-wider">{t("contact.message")}</label>
                  <textarea form="contact-form" required rows={5} className="w-full px-4 py-3 border border-border bg-background text-foreground text-[14px] focus:outline-none focus:ring-1 focus:ring-ring placeholder:text-muted-foreground/40 resize-none transition-colors" placeholder={t("contact.message.placeholder")} />
                  <div className="mt-5">
                    <button type="submit" form="contact-form" className="inline-flex items-center justify-center gap-2 px-7 py-3 bg-foreground text-background font-medium text-[14px] hover:bg-foreground/90 transition-colors">
                ...
                {/* Contact info: Email, Location, Phone, Twitter */}
